...
export function OfferCard({ offer }: OfferCardProps) {
  const discountedPrice = offer.discount ? offer.price * (1 - offer.discount / 100) : offer.price
      ...
      <div className="p-4">
        <div className="flex justify-between items-start mb-2">
          ...
          <div className="text-right">
            {offer.discount && (
              <span className="block text-sm text-gray-500 line-through">${offer.price.toFixed(2)}</span>
            )}
            <span className="text-lg font-bold text-gray-900">${discountedPrice.toFixed(2)}</span>
          </div>
        </div>
        <p className="text-gray-600 text-sm mb-3">{offer.shortDescription}</p>
